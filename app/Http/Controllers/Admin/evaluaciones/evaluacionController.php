        ...
        if ($request->ajax()) {
            ...
            if (isset($data['busquedaPersonalizada'])) {
                ...
                if ($data['id_carrera'] != 0) {
                    $WHERE .= " AND cuest.carrera=" . $data['id_carrera'];
                ...
                if ($request->get('semestre') != null && $request->get('semestre') != 0) {
                    $semestre = $request->get('semestre');
                    $WHERE .= " AND cuest.semestre={$semestre}";
                }
                if ($request->get('turno') != null && $request->get('turno') != 0) {
                    $turno = $request->get('turno');
                    $WHERE .= " AND cuest.turno={$turno}";
                }
                if ($request->get('grupo') != null && $request->get('grupo') != 0) {
                    $grupo = $request->get('grupo');
                    $WHERE .= " AND cuest.grupo={$grupo}";
                }

                if (count($dataCarrera) > 0) {
                    $SQL = "SELECT cuest.*,us.nombre,us.ap_paterno,us.ap_materno,dtalumno.matricula
                        FROM cuestionario AS cuest
                        INNER JOIN users AS us	ON cuest.id_user_alumno=us.id
                        INNER JOIN datos_alumnos AS dtalumno ON cuest.id_user_alumno=dtalumno.user_id_alumno
            ...
            if (count($cuestionario) > 0) {
                $respuestasCuestionario = count($cuestionario);
                ...
                foreach ($cuestionario as $index_item => $item) {
                    ...
                    foreach ($respuestas_cuestionario as $key => $respuesta) {
                        ...
                        $cuestionario[$index_item]->{'c_siempre'} = isset($cuestionario[$index_item]->{'c_siempre'}) ? $cuestionario[$index_item]->{'c_siempre'} +$c_siempre : $c_siempre;
                        $cuestionario[$index_item]->{'c_casi_siempre'} = isset($cuestionario[$index_item]->{'c_casi_siempre'}) ? $cuestionario[$index_item]->{'c_casi_siempre'} +$c_casi_siempre : $c_casi_siempre;
                        $cuestionario[$index_item]->{'c_a_veces'} = isset($cuestionario[$index_item]->{'c_a_veces'}) ? $cuestionario[$index_item]->{'c_a_veces'} +$c_a_veces : $c_a_veces;
                        $cuestionario[$index_item]->{'c_nunca'} = isset($cuestionario[$index_item]->{'c_nunca'}) ? $cuestionario[$index_item]->{'c_nunca'} +$c_nunca : $c_nunca;
            ...
            return json_encode([
                'status' => 200, 
                'preguntas' => $preguntas,
                'respuestasFrecuentes' => $arreglo_respuesta2,
                'sql' => $SQL,
