    ...
    public function index(Request $request){
      ...
      if($request->ajax()){
        ...
        if($tipo_usuario=="alumno"){
            ...
            if($SEMESTRE!=""){
                $users->where('datos_alumnos.semestre', '=',$SEMESTRE);
            }
            if($CARRERA!=""){
                $users->where('datos_alumnos.carrera', '=',$CARRERA);
            }
            if($PERIODO!=""){
                $users->where('datos_alumnos.periodo', '=',$PERIODO);
            }

            $users->orderBy('users.id','desc')->get();
            ...
            if($SEMESTRE!=""){
                $WHERE.=" OR datos_alumnos.semestre=".$SEMESTRE;
            }
            if($CARRERA!=""){
                $WHERE.="  OR datos_alumnos.carrera='$CARRERA'";
            }
            if($PERIODO!=""){
                $WHERE.="  OR datos_alumnos.periodo='$PERIODO'";
            }
            // if($TURNO!=""){
            //     $WHERE.="  OR datos_alumnos.turno=".$TURNO;
            // }
            if($GRUPO!=""){
                $WHERE.="  OR datos_alumnos.grupo='$GRUPO'";
            }
            if($MATRICULA!=""){
                $WHERE.=" OR datos_alumnos.matricula='$MATRICULA'";
            }

            $SQL_ALUMNO=" select * from users INNER JOIN datos_alumnos ON users.id=datos_alumnos.user_id_alumno
        ...
        if($tipo_usuario!="alumno"&& $tipo_usuario!="administrador"){
            ...
            $SQL_ALUMNO=" select * from users INNER JOIN datos_docentes ON users.id=datos_docentes.user_id_docente
        ...
        return json_encode(['data'=>$users,'status'=>400,'SQL'=>'']);
      }
