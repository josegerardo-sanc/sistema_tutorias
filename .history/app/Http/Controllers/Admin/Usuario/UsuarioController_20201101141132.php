      ...
      if($request->ajax()){
        ...
        if($tipo_usuario!="alumno"&& $tipo_usuario!="administrador"){
            ...
            if($CEDULA_PROFESIONAL!=""){
                $WHERE.=" AND datos_alumnos.matricula='$CEDULA_PROFESIONAL'";
            }
        ...
        if($user==null || empty($user)){
           return json_encode(['status'=>400,'info'=>'NO SE ENCONTRO ESTE USUARIO']);
            ...
            if($user->tipo_usuario=="alumno"){
                ...
                ->join('datos_alumnos', 'users.id', '=', 'datos_alumnos.user_id_alumno')
                ->select('users.*', 'datos_alumnos.*','datos_alumnos.id AS id_alumno')
                ->where('users.id','=',$id)
                ->get();
            }else{
                $usersData = DB::table('users')
                ->join('datos_docentes', 'users.id', '=', 'datos_docentes.user_id_docente')
                ->select('users.*', 'datos_docentes.id AS id_docente')
                ->where('users.id','=',$id)
                ->get();
            }
        ...
        try {
         ...
         $validatedData = Validator::make($data, [
          ...
          if($tipo_usuario=="alumno"){
              // alumno
              $validatedDatos_complementarios = Validator::make($data, [
                  'matricula'=>['required','string','max:10','sometimes','unique:datos_alumnos,matricula,'.$user->id_alumno],
                  'semestre_escolar'=>'required',
                  'carrera_escolar'=>'required',
                  'periodo_escolar'=>'required',
                  'turno_escolar'=>'required',
          ...
          if($tipo_usuario!="alumno" && $tipo_usuario!="administrador"){
              // diferente de alumno y administardior, solictar cedula profesional etc.
              $validatedDatos_complementarios = Validator::make($data, [
                  'cedula_profesional'=>['required','string','max:10','sometimes','unique:datos_docentes,cedula_profesional,'.$user->id_docente]
                  //'grupo_escolar'=>'required'
              ]);
          }
          ...
          try {
              ...
              if($tipo_usuario=="alumno"){
                  $status=DB::table('datos_alumnos')
                  ->where('id',$user->id_alumno)
                  ->update([
                          'matricula'=>$matricula,
                          'periodo'=> $periodo_escolar ,
                          'semestre'=>$semestre_escolar ,
                ...
                if(!$status){
                    throw new Exception("NO SE PUDO REALIZAR  LA ACTUALIZACION DE DATOS DEL ALUMNO, EXEPTION");
                }
              }
              if($tipo_usuario!="alumno" && $tipo_usuario!="administrador"){
                  $status=DB::table('datos_docentes')
                  ->where('id',$user->id_docente)
                  ->update([
                          'cedula_profesional'=>$cedula_profesional
                  ]);
                  if(!$status){
                   throw new Exception("NO SE PUDO REALIZAR LA ACTUALIZACION DE DATOS DEL DOCENTE, EXEPTION");
                  }
              }
  
             DB::commit();
              return json_encode(['status'=>"200",'info'=>"Datos actualizado con exitoso"]);
